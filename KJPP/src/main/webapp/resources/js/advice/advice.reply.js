$(document).ready(function(){
   ...
   function selectData(pageNum, num){
      ...
      $.ajax({
         type:'post',
         data:{pageNum:pageNum, adv_num:num},
         url:'listReply.do',
         dataType:'json',
         cache:false,
         timeout:30000,
   ...
   $('.paging-button input').click(function(){
	  ...
	  selectData(pageNum, $('#adv_num').val());
   });
   ...
   $(document).on('click','.modify-btn',function(){
	  //댓글 글번호
	   var ar_num = $(this).attr('data-num');
	  //작성자 아이디
	   var email= $(this).attr('data-id');
	  //댓글 내용
	   var content = $(this).parent().find('p').html().replace(/<br>/gi,'\n');
	   //댓글 수정폼 UI
	   var modifyUI = '<form id="adv_form">';
   	   modifyUI += '   <input type="hidden" name="ar_num" id="ar_num" value="'+ar_num+'">';
   	   modifyUI += '   <input type="hidden" name="email" id="mre_email" value="'+email+'">';
   	   modifyUI += '   <textarea rows="3" cols="50" name="ar_comment" id="mre_content" class="rep-content">'+content+'</textarea>';
   	   modifyUI += '   <div id="mre_first"><span class="letter-count">300/300</span></div>';
	   modifyUI += '   <div id="mre_second" class="align-right">';
	   modifyUI += '   	<input type="submit" class="btn btn-default" value="수정">';
	   modifyUI += '   	<input type="button" class="re-reset btn btn-default" value="취소">';
	   modifyUI += '   </div>';
	   ...
	   //문서 개게에 반영
	   $('#mre_fist .letter-count').text(remain);
   });
   ...
   //댓글 수정  
   $(document).on('submit', '#adv_form', function(event){
	  if($('#mre_content').val()==''){
		  alert('내용을 입력하세요');
		  $('#mre_content').focus();
		  return false;
	  ...
	  $.ajax({
		 ...
		 success:function(data){
			 if(data.result == 'logout'){
				 ...
				 $('#adv_form').parent().find('p').html($('#mre_content').val().replace(/\n/g,'<br>'));
				 //수정폼 초기화
				 initModifyForm();
   ...
   $(document).on('click', '.delete-btn', function(){
      ...
      $.ajax({
         type:'post',
         url:'deleteReply.do',
         data:{ar_num:ar_num, email:email},
         dataType:'json',
         cache:false,
         timeout:30000,
         success:function(data){
