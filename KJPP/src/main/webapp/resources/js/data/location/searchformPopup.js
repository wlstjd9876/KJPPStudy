			...
			success: function(data){
				...
				if(myItem!=undefined){
					for(var i=0; i<myItem.length; i++){
						...
						output += '				<p><a href="'+contextPath+'/data/location/viewPopup.do?contentId='+myItem[i].contentid+'&contenttypeid='+myItem[i].contenttypeid+'" class="btn btn-primary hn">자세히 보기</a></p>';
						output += '			</div>';
						output += '	</div>';
						output += '</div>';
					}
